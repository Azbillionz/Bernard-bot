...
import { startPumpfunListener, stopPumpfunListener } from "./pumpfun";
...
export async function handleAutoSnipe(ctx: Context): Promise<void> {
  ...
  // This is the actual fix: turning Auto-Snipe ON must start the listener
  // (previously only the separate 🌱 PumpFun screen did this — flipping
  // this toggle alone never made the bot listen for anything).
  ...
  const statusLine = newState
    ? `🟢 <b>Auto-Snipe is ON</b> — listener active, watching for new launches`
